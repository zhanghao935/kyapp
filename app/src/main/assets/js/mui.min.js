...
function() {
	...
	function(a, b, c) {
		if((a.os.android || a.os.ios) && !b.FastClick) {
			var d = function(a, b) {
				return "LABEL" === b.tagName && b.parentNode && (b = b.parentNode.querySelector("textarea")), !b || "radio" !== b.type && "checkbox" !== b.type || b.disabled ? !1 : b
			};
			a.registerTarget({
				name: c,
				index: 40,
	...
	function(a, b, c) {
		var d = "mui-popup",
			...
			r = function(a) {
				return '<div class="' + k + '"><textarea autofocus placeholder="' + (a || "") + '"/></textarea></div>'
			},
			s = function(a, b, c) {
	...
	function(a, b, c) {
		...
		t.prototype.init = function() {
			...
			if(this.sliderActionClass) {
				...
				b.addEventListener("textarea", e), b.addEventListener("tap", e), b.addEventListener(a.EVENT_MOVE, function(a) {
					a.stopPropagation()
				})
			} else {
				if(this.clearActionClass) {
					...
					if(!f) return;
					a.each(["keyup", "change", "textarea", "focus", "cut", "paste"], function(a, c) {
						! function(a) {
			...
			if(b.plus) {
				...
				e.element.value = "", c.body.classList.add(m), plus.speech.startRecognize({
					...
					}), f !== e.element.value && (a.trigger(e.element, "change"), a.trigger(e.element, "textarea"))
				}, function(a) {
			...
			return this.each(function() {
				var b = null,
					d = [],
					e = s(this.parentNode);
				if("range" === this.type && e.classList.contains("mui-textarea-range")) d.push("slider");
				else {
					var f = this.classList;
					f.contains("mui-textarea-clear") && d.push("clear"), a.os.android && a.os.stream || !f.contains("mui-textarea-speech") || d.push("speech"), f.contains("mui-textarea-password") && d.push("password"), "search" === this.type && e.classList.contains("mui-search") && d.push("search")
				}
				var g = this.getAttribute("data-textarea-" + d[0]);
				if(g) b = a.data[g];
				else {
					g = ++a.uuid, b = a.data[g] = new t(this, {
						actions: d.join(",")
					});
					for(var h = 0, i = d.length; i > h; h++) this.setAttribute("data-textarea-" + d[h], g)
				}
				c.push(b)
			}), 1 === c.length ? c[0] : c
		}, a.ready(function() {
			a(".mui-textarea-row textarea").textarea()
		})
	}(mui, window, document),
	function(a, b) {
		var c = "mui-active",
	...
	function(a) {
		var b = "ontouchstart" in document,
			...
			h = ".mui-textarea-numbox,.mui-numbox-textarea",
			i = a.Numbox = a.Class.extend({
				init: function(b, c) {
					var d = this;
					if(!b) throw "构造 numbox 时缺少容器元素";
					d.holder = b, c = c || {}, c.step = parseInt(c.step || 1), d.options = c, d.textarea = a.qsa(h, d.holder)[0], d.plus = a.qsa(f, d.holder)[0], d.minus = a.qsa(g, d.holder)[0], d.checkValue(), d.initEvent()
				},
				initEvent: function() {
					var b = this;
					b.plus.addEventListener(c, function(c) {
						var e = parseInt(b.textarea.value) + b.options.step;
						b.textarea.value = e.toString(), a.trigger(b.textarea, d, null)
					}), b.minus.addEventListener(c, function(c) {
						var e = parseInt(b.textarea.value) - b.options.step;
						b.textarea.value = e.toString(), a.trigger(b.textarea, d, null)
					}), b.textarea.addEventListener(d, function(c) {
						b.checkValue();
						var e = parseInt(b.textarea.value);
				...
				getValue: function() {
					var a = this;
					return parseInt(a.textarea.value)
				},
				checkValue: function() {
					var a = this,
						b = a.textarea.value;
					if(null == b || "" == b || isNaN(b)) a.textarea.value = a.options.min || 0, a.minus.disabled = null != a.options.min;
					else {
						var b = parseInt(b);
						null != a.options.max && !isNaN(a.options.max) && b >= parseInt(a.options.max) ? (b = a.options.max, a.plus.disabled = !0) : a.plus.disabled = !1, null != a.options.min && !isNaN(a.options.min) && b <= parseInt(a.options.min) ? (b = a.options.min, a.minus.disabled = !0) : a.minus.disabled = !1, a.textarea.value = b
					}
				},
				...
				setValue: function(a) {
					this.textarea.value = a, this.checkValue()
				}
			});
